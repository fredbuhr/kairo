...
MEMORY_PROJECT_ID = uuid.uuid5(uuid.NAMESPACE_URL, "kairo:project:memory")
...
def _task_id(message_id: uuid.UUID, generation: int) -> uuid.UUID:
    return uuid.uuid5(
        uuid.NAMESPACE_URL,
        f"kairo:memory:conversation-message:{message_id}:v{MEMORY_SOURCE_VERSION}:g{generation}",
    )
...
async def ensure_memory_projection(
    ...
    correlation_id = uuid.uuid5(
        uuid.NAMESPACE_URL,
        f"kairo:memory-projection:{message_id}:v{MEMORY_SOURCE_VERSION}:g{requested_generation}",
    )
...
async def report_memory_projection(
    ...
    correlation_id = uuid.uuid5(
        uuid.NAMESPACE_URL,
        f"kairo:memory-projection:{message_id}:v{MEMORY_SOURCE_VERSION}:g{generation}",
    )
    await enqueue_domain_event(
        session,
...
async def rebuild_memory_projections(
    ...
    for message in messages:
        ...
        correlation_id = uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"kairo:memory-rebuild:{message.id}:v{MEMORY_SOURCE_VERSION}:g{next_generation}",
        )
        await enqueue_domain_event(
            session,
