...
async def reserve_model_call(
    ...
    if await session.scalar(live) >= settings.kairo_model_global_concurrency:
        raise HTTPException(429, "model_global_capacity_exhausted", headers={"Retry-After": "1"})
    if await session.scalar(live.where(ModelReservation.owner_subject == owner)) >= settings.kairo_model_owner_concurrency:
    ...
    if await daily_exposure(session) + amount > settings.kairo_model_global_daily_budget_usd:
        return "model_global_daily_budget_exceeded"
    if await daily_exposure(session, owner) + amount > settings.kairo_model_owner_daily_budget_usd:
...
async def owner_admission(
    principal: Principal = Depends(require_kairo_user), session: AsyncSession = Depends(get_session),
) -> dict:
    ...
    return {"active_calls": active, "concurrency_limit": settings.kairo_model_owner_concurrency,
            "daily_exposure_usd": str(await daily_exposure(session, principal.subject)),
            "daily_budget_usd": str(settings.kairo_model_owner_daily_budget_usd)}
