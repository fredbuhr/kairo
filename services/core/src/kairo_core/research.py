...
class ResearchCommandInput(BaseModel):
    """Semantic/deterministic routing contract: no authority-bearing execution fields."""

    query: str = Field(min_length=3, max_length=4000)
    max_tool_calls: int = Field(default=3, ge=1, le=8)


class ResearchRunCreate(ResearchCommandInput):
...
async def start_research_tool(
    ...
    if parent_project is None:
        raise HTTPException(status_code=409, detail="Research Project ownership binding is missing")
    owner_subject = parent_project.keycloak_subject
    ...
    existing = await session.scalar(
        select(ToolInvocation).where(
            ToolInvocation.keycloak_subject == owner_subject,
    ...
    child_task_id = uuid.uuid5(uuid.NAMESPACE_URL, f"kairo:research-tool-task:{invocation_id}")
    correlation_id = uuid.uuid5(uuid.NAMESPACE_URL, f"kairo:research-tool-correlation:{invocation_id}")
    child = Task(
        id=child_task_id,
...
async def research_tool_result(
    ...
    if (
        project is None
        or project.keycloak_subject != invocation.keycloak_subject
