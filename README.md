...
**Phase:** V0 foundations / architecture.

No production deployment exists yet. The current goal is to validate the smallest architecture that can support durable memory, autonomous background work, multi-project context, and a responsive interface without creating an unnecessary stack of services.
...
- **Model-agnostic intelligence.** OpenAI and Anthropic are initial inference providers, not the identity or memory of KAIRO.
...
The actual runtime workspace will be created outside the Git repository. Personal memory and the live user profile are runtime data, not source code.

## Near-term milestone

The first software milestone is intentionally small:

> From a KAIRO conversation, create a durable, structured idea inside a project and retrieve it later with its provenance intact.

The second milestone is the one that separates KAIRO from a chat UI:

> Schedule an approved research task, close every client, let the server finish the job, and surface the result in the next morning brief.
