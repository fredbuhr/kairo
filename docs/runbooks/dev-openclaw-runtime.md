...
The development dependency on `@kairo/core` is a local `file:` dependency, which npm represents as a symlink in the source checkout. OpenClaw correctly rejects a plugin source tree when a dependency symlink crosses the plugin boundary. KAIRO therefore bundles `@kairo/core` into the npm package and installs the packed tarball rather than the source directory.

Return to the plugin directory, rebuild, then create the installable package:

```bash
cd plugins/openclaw-kairo-tools
npm run plugin:build
KAIRO_PLUGIN_TGZ="$(npm pack --silent)"
```

Confirm the packed archive contains a real bundled copy of KAIRO Core rather than an external symlink:

```bash
tar -tzf "$KAIRO_PLUGIN_TGZ" | grep 'package/node_modules/@kairo/core/package.json'
```

Install the packed plugin through OpenClaw's plugin manager:

```bash
"$OPENCLAW_BIN" plugins install "$PWD/$KAIRO_PLUGIN_TGZ" --accept-capabilities
```
...
The generated `.tgz` file is a local build artifact. Do not commit it.
